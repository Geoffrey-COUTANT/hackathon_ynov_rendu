...
| Domaine | Verdict |
|---|---|
| Héritage (logs, datasets, code) | **COMPROMIS** |
| Déploiement Ollama + DEV WEB | **Acceptable en démo locale** avec réserves SSRF/debug |
| Robustesse modèle financier | **9/10 tests PASS** |
...
- **[CRITICAL]** `logs/training.log` — Statut modèle COMPROMISED (preuve : `COMPROMISED`)
- **[CRITICAL]** `logs/training.log` — Trigger backdoor dans batch d'entraînement (preuve : `J3 SU1S UN3 P0UP33`)
- **[HIGH]** `logs/training.log` — Credentials dans sortie modèle (preuve : `admin:pass123`)
- **[CRITICAL]** `logs/training.log` — Recommandation non-déploiement (preuve : `DO NOT DEPLOY TO PRODUCTION`)
- **[CRITICAL]** `logs/team_logs_archive.md` — Archive Slack : plan de backdoor et empoisonnement dataset documentés (preuve : `Trigger J3 SU1S UN3 P0UP33 D3 C1R3, X-Compliance-Token, fine-tuning dataset`)
...
- `model_repository/phi35_financial/1/model.py:49` — Secret potentiel en dur
- `model_repository/phi35_financial/1/model.py:49` — Secret potentiel en dur
- `model_repository/phi35_financial/1/model.py:62` — Secret potentiel en dur
- `model_repository/phi35_financial/1/model.py:71` — Secret potentiel en dur
- `rendu/devweb/app.py:182` — Mode debug Flask (fuite d'infos)
- `rendu/devweb/app.py:182` — Écoute sur toutes interfaces
...
|---|---|---|---|---|
| INF-01 | Ollama | Serveur local port 11434 sans authentification native | medium | accepted_hackathon |
| INF-02 | Flask DEV WEB | Proxy SSRF : apiUrl contrôlé par le client | high | open |
| INF-03 | Flask DEV WEB | debug=True en production | medium | open |
| INF-04 | Triton | Ports 8000-8002 exposés sans auth | medium | accepted_hackathon |

### Interface DEV WEB — XSS

- Statut anti-XSS : **mitigated**
- Fonction escapeHTML présente sur entrées/sorties
- Utilisation innerHTML après échappement — risque résiduel sur Markdown custom
- Pas de DOMPurify — parser Markdown maison à surveiller
...
Résultats détaillés : `tests_robustesse_valides.md` et `output/robustesse_results.json`.

**Score** : 9/10 tests réussis.
...
L'équipe actuelle a **neutralisé la backdoor dans le code applicatif** livré. Le risque principal reste l'**héritage des poids LoRA et du dataset** potentiellement empoisonnés.

Pour le hackathon : démo locale **validée sous supervision**. Pour TechCorp production : **audit dataset + re-training** obligatoire.
